...
class Command(BaseCommand):
    help = 'Changes supervisor privileges for a user.'

    def add_arguments(self, parser):
        parser.add_argument('--user', type=str, required=True)
        parser.add_argument('--supervisor', type=bool, required=True)
    ...
    def handle(self, *args, **kwargs):
        ...
        user = CustomUser.objects.filter(first_name=first_name, last_name=last_name).exists()

        if kwargs['supervisor']:
